  ...
  <div class="container">
    ...
    <div class="collapse navbar-collapse" id="navbarToggler">
      <ul class="navbar-nav ml-lg-5 mt-3 mt-lg-0">
        <li class="nav-item">
          <a class="nav-link {{request()->is('/') ? 'active' : ''}}" href="{{route('home')}}">{{ __('index.home') }}</a>
        </li>
        @if(!empty($user))
        <li class="nav-item">
          <a class="nav-link {{request()->is('dashboard') ? 'active' : ''}}" href="{{route('dashboard')}}">{{ __('index.dashboard') }}</a>
        </li>
        @endif
      </ul>
      @if(empty($user))
      <div class="ml-auto my-2 my-lg-0">
        <button class="btn btn-dark rounded-pill" data-toggle="modal" data-target="#loginModal">{{ __('index.login') }}</button>
      </div>
      @endif
    </div>
  </div>
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-body">
        <form>
          <h2 class="title"> {{ __('index.login') }}</h2>
